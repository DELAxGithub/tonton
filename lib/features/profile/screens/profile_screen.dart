...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _saveName(String value) async {
    ...
    if (value.trim().length > 20) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ニックネームは20文字以内で入力してください')),
      );
      return;
    }
    ...
    await ref.read(userProfileProvider.notifier).updateDisplayName(value.trim());
    setState(() {
      _isEditing = false;
      _savingField = null;
    });
  ...
  Future<void> _saveGoal(String value) async {
    ...
    if (goal < 1000) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('月間目標は1000kcal以上で設定してください')),
      );
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('月間目標を更新しました')),
      );
    }
  }
  ...
  Future<void> _saveWeight(String value) async {
    ...
    if (weight <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('体重は0より大きい値を入力してください')),
      );
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('体重を更新しました')),
      );
    }
  }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('体の変化', style: Theme.of(context).textTheme.bodySmall),
                          Text(
                            userProfile.ageGroup == 'young' ? '若い頃と変わらない' :
                            userProfile.ageGroup == 'middle' ? '脂肪が増えやすくなった' :
                            userProfile.ageGroup == 'senior' ? '健康が気になってきた' : '未設定',
                            style: Theme.of(context).textTheme.bodyMedium,
                          ),
                        ],
                      ),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _savingField == 'goal' && _isEditing
                        ...
                        : IconButton(
                            icon: const Icon(Icons.save),
                            onPressed: () => _saveGoal(_goalController.text),
                          ),
                  ),
                  // onChangedを削除して自動保存を無効化
  ...
  Widget _buildNutritionBalanceCard(BuildContext context, WidgetRef ref) {
    ...
    // 性別と年齢層の表示テキスト
    final genderText = userProfile.gender == 'male' ? '男性' : 
                      userProfile.gender == 'female' ? '女性' : '未設定';
    final ageGroupText = userProfile.ageGroup == 'young' ? '若い頃と変わらない' :
                        userProfile.ageGroup == 'middle' ? '脂肪が増えやすくなった' :
                        userProfile.ageGroup == 'senior' ? '健康が気になってきた' : '未設定';
      ...
      child: Column(
        ...
        children: [
          ...
          if (autoPfc != null) ...[
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text('体重: ${userProfile.weight?.toStringAsFixed(1) ?? "未設定"} kg',
                          style: Theme.of(context).textTheme.bodySmall),
                    ],
                  ),
                ],
            ...
            // 自動計算されたPFC値
            Text('目標カロリー: $dailyTarget kcal/日',
                style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                      fontWeight: FontWeight.bold,
                    )),
            const SizedBox(height: Spacing.sm),
            
            _buildPfcRow(context, 'たんぱく質', autoPfc.protein, 
                        autoPfc.protein * 4, TontonColors.proteinColor),
            const SizedBox(height: 8),
            _buildPfcRow(context, '脂質', autoPfc.fat, 
                        autoPfc.fat * 9, TontonColors.fatColor),
            const SizedBox(height: 8),
            _buildPfcRow(context, '炭水化物', autoPfc.carbohydrate, 
                        autoPfc.carbohydrate * 4, TontonColors.carbsColor),
  ...
  Widget _buildPfcRow(BuildContext context, String label, double grams, 
                      double calories, Color color) {
    final percentage = (calories / ref.read(dailyCalorieTargetProvider) * 100).round();
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        Text('(${calories.round()}kcal / $percentage%)',
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                )),
      ],
