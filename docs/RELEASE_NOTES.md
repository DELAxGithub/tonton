...
#### アカウント削除機能の実装
- **Apple ガイドライン5.1.1対応**: アプリ内でのアカウント削除機能を実装
- プロフィール画面のデータ管理セクションにアカウント削除ボタンを追加
- 確認ダイアログによる誤削除防止機能
- 削除時の影響範囲を明確に表示

#### サポートページの新設
- **新規サポートページ**: https://delaxgithub.github.io/tonton/support/
- アカウント削除手順の詳細説明
- FAQ形式でのよくある質問への回答
- お問い合わせ情報とサポート時間の明記
...
#### 摂取カロリー表示の修正
- **問題**: ヒストリー画面で摂取カロリーが0と表示される重大なバグを修正
- **原因**: データ取得ロジックとキャッシュの不整合
- **修正**: 食事記録の直接処理とキャッシュ無効化メカニズムの追加
- **結果**: 貯金額の計算が正確に表示されるように改善

### 🔧 技術的改善
- 食事記録変更時の自動キャッシュクリア機能
- protected メンバーへの不正アクセスを解消
- Flutter analyzeエラーをすべて修正
- iOS リリースビルドの成功確認
