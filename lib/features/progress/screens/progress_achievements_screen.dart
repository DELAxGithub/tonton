...
class ProgressAchievementsScreen extends ConsumerWidget implements AppPage {
      ...
      data: (_) {
        ...
        return provider_pkg.Consumer<HealthProvider>(
          builder: (context, hp, child) {
            // Get weight records from weight history provider
            final weightRecordsAsync = ref.watch(weightHistoryProvider(records));
            
            return weightRecordsAsync.when(
              loading: () => const Center(child: CircularProgressIndicator()),
              error: (e, _) => _buildContentWithNullWeights(records, ref, context),
              data: (weightRecords) {
  ...
  Widget _buildContentWithNullWeights(
    List<CalorieSavingsRecord> records,
    WidgetRef ref,
    BuildContext context,
  ) {
    // Fallback to null weights when error occurs
