...
void main(List<String> arguments) async {
  ...
  final parser = ArgParser()
    ...
    ..addOption('url', abbr: 'u', help: 'Supabase URL (optional if env vars are set)')
    ..addOption('key', abbr: 'k', help: 'Supabase Anon Key (optional if env vars are set)')
  ...
  try {
    ...
    final supabaseUrl = args['url'] as String? ??
        Platform.environment['SUPABASE_URL'] ??
        promptForInput('Enter Supabase URL:');
    
    final supabaseAnonKey = args['key'] as String? ??
...
void printUsage(ArgParser parser) {
  print('Usage: dart test_edge_function.dart --image=path/to/image.jpg [--url=SUPABASE_URL] [--key=SUPABASE_ANON_KEY]');
  print(parser.usage);
}

/// Test the Edge Function with the given image
Future<void> testEdgeFunction(String imagePath, String supabaseUrl, String supabaseAnonKey) async {
  ...
  try {
    ...
    print('Request completed in: ${duration.inSeconds}.${duration.inMilliseconds % 1000} seconds');
