  ...
  final parser =
      ArgParser()
        ...
        ..addOption(
          ...
          defaultsTo: 'processed',
        )
        ..addOption(
          'width',
          abbr: 'w',
          help: 'Max width for resized image',
          defaultsTo: '1024',
        )
        ..addOption(
          'quality',
        ...
        ..addFlag(
          'info',
          help: 'Only show image info without processing',
          negatable: false,
        )
        ..addFlag(
          'help',
...
void printUsage(ArgParser parser) {
  stdout.writeln(
    'Usage: dart test_image_processing.dart --image=path/to/image.jpg [options]',
  );
...
Future<void> processImage(
  ...
  stdout.writeln(
    '- JPEG (${(jpegBytes.length / 1024).toStringAsFixed(2)} KB): ${jpegOutput.path}',
  );
  stdout.writeln(
    '- PNG (${(pngBytes.length / 1024).toStringAsFixed(2)} KB): ${pngOutput.path}',
  );
  ...
  stdout.writeln(
    '- JPEG size: ${(jpegBytes.length / 1024).toStringAsFixed(2)} KB (${(100 * jpegBytes.length / imageBytes.length).toStringAsFixed(1)}%)',
  );
  stdout.writeln(
    '- PNG size: ${(pngBytes.length / 1024).toStringAsFixed(2)} KB (${(100 * pngBytes.length / imageBytes.length).toStringAsFixed(1)}%)',
  );

  if (resized) {
    stdout.writeln(
      '- Resized: Yes (from ${originalImage.width}x${originalImage.height} to ${processedImage.width}x${processedImage.height})',
    );
  } else {
