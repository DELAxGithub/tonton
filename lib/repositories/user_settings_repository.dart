...
class UserSettingsRepository {
  ...
  static const double _defaultMonthlyTarget =
      14400; // Default: ~0.5kg weight loss per month (500 cal/day)
  ...
  Future<double> getMonthlyTargetNetBurn() async {
    try {
      ...
      if (target != null) {
        developer.log(
          'Retrieved monthly target: $target kcal',
          name: 'TonTon.UserSettingsRepository',
        );
        return target;
      } else {
        developer.log(
          'No monthly target set, using default: $_defaultMonthlyTarget kcal',
          name: 'TonTon.UserSettingsRepository',
        );
  ...
  Future<bool> setMonthlyTargetNetBurn(double target) async {
    try {
      ...
      if (success) {
        developer.log(
          'Set monthly target to: $target kcal',
          name: 'TonTon.UserSettingsRepository',
        );
      } else {
        developer.log(
          'Failed to set monthly target',
          name: 'TonTon.UserSettingsRepository',
        );
      }
