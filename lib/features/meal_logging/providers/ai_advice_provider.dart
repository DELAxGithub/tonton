...
class AiAdviceNotifier extends StateNotifier<AsyncValue<AiAdviceResponse?>> {
  ...
  AiAdviceNotifier(this._service, this._ref)
  ...
  Future<void> fetchAdvice(List<MealRecord> meals, String languageCode) async {
    ...
    try {
      ...
      final targetPfcRatio =
          autoPfc != null
              ? PfcRatio(
                protein:
                    autoPfc.protein /
                    (autoPfc.protein + autoPfc.fat + autoPfc.carbohydrate),
                fat:
                    autoPfc.fat /
                    (autoPfc.protein + autoPfc.fat + autoPfc.carbohydrate),
                carbohydrate:
...
final weeklyAchievementTrendProvider = Provider<Map<String, dynamic>>((ref) {
  ...
  // 平均達成率を計算
  final avgCalories =
      dailyAchievements.map((d) => d['calories']!).reduce((a, b) => a + b) /
      dailyAchievements.length;
  final avgProtein =
      dailyAchievements.map((d) => d['protein']!).reduce((a, b) => a + b) /
      dailyAchievements.length;
  final avgFat =
      dailyAchievements.map((d) => d['fat']!).reduce((a, b) => a + b) /
      dailyAchievements.length;
  final avgCarbs =
      dailyAchievements.map((d) => d['carbs']!).reduce((a, b) => a + b) /
      dailyAchievements.length;
  ...
  if (dailyAchievements.length >= 6) {
    ...
    final secondHalf =
        dailyAchievements
            .skip(dailyAchievements.length - 3)
            .map((d) => d['calories']!)
            .reduce((a, b) => a + b) /
        3;
