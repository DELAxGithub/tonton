...
class HealthProvider with ChangeNotifier {
  ...
  bool get hasData => _todayActivity != null || _yesterdayActivity != null || _yesterdayWeight != null;
  ...
  Future<bool> requestPermissions() async {
    ...
    try {
      ...
      developer.log('Permissions result: $_hasPermissions', name: 'TonTon.HealthProvider');
      ...
      developer.log('Error requesting permissions: $e', name: 'TonTon.HealthProvider.error', error: e, stackTrace: stack);
      _setStatus('エラー: $e');
      return false;
    } finally {
      _setLoading(false);
  ...
  Future<void> fetchAllData() async {
    developer.log('fetchAllData called, isLoading: $_isLoading', name: 'TonTon.HealthProvider');
    ...
    if (!_hasPermissions) {
      developer.log('No permissions yet, requesting now', name: 'TonTon.HealthProvider');
      final hasPermissions = await requestPermissions();
      if (!hasPermissions) {
        developer.log('Permission request failed', name: 'TonTon.HealthProvider');
        _setLoading(false);
        return;
      }
    }
    ...
    try {
      developer.log('Fetching today activity summary', name: 'TonTon.HealthProvider');
      // Get today's data
      _todayActivity = await _healthService.getTodayActivitySummary();
      developer.log('Today activity summary fetched: ${_todayActivity != null}', name: 'TonTon.HealthProvider');
      
      // Get yesterday's data
      final now = DateTime.now();
      final yesterday = DateTime(now.year, now.month, now.day - 1);
      
      developer.log('Fetching yesterday activity summary and weight', name: 'TonTon.HealthProvider');
      ...
      developer.log('Error fetching health data: $e', name: 'TonTon.HealthProvider.error', error: e, stackTrace: stack);
      _setStatus('エラー: $e');
    } finally {
      _setLoading(false);
    }
  ...
  void _setLoading(bool loading, [String? message]) {
    developer.log('Setting loading: $loading${message != null ? ", message: $message" : ""}', name: 'TonTon.HealthProvider');
    _isLoading = loading;
    if (message != null) {
      _statusMessage = message;
    }
    notifyListeners();
    developer.log('notifyListeners called for loading change', name: 'TonTon.HealthProvider');
  }
  ...
  void _setStatus(String message) {
    ...
    _statusMessage = message;
    notifyListeners();
    developer.log('notifyListeners called for status change', name: 'TonTon.HealthProvider');
  }
